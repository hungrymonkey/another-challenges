...
Skip List: Since memmove is expensive for moving huge chucks of memmory, LinkedList has O(1) inserts. Neverless, linkedlist needs to be traversed linearly. Search a linkedList needs to be O(logn) or faster to have an overall performance of O(nlogn). A Skip List is essentially a binary searched linkedlist.
